...
이러한 문제를 해결하기 위해 객체지향 언어에서는 late 바인딩 개념을 사용한다(late 바인딩은 동적 바인딩이라고도 한다). 즉, 어떤 파생 타입의 move()를 호출할 것인지를 실행할 때 결정하는 것이다. [컴파일러에서는](https://github.com/nara1030/ThisIsJava/blob/master/docs/etc/polymorphism.md) 단지 그러한 함수가 있는지, 그리고 함수의 인자와 반환 타입이 맞는지만`(?)`을 확인한다. 어떤 move()를 호출해야 하는지를 모르는 것이다.
