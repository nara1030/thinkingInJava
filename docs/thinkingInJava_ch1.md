...
* 배경: 파생 타입의 객체를 베이스 타입으로 취급해야 한다!
...
* 해결방법: 파생 타입의 객체를 베이스 타입으로 취급하면 문제가 생기는데?
	...
	2. 파생 타입의 객체를 베이스 타입으로 지정하되, 베이스 타입의 함수를 호출하면 그 객체의 타입을 알아내어 그 객체가 속한 파생 타입에서 오버라이딩한 함수를 실행
		* 하지만 이러한 일을 컴파일러가 해주진 못함
			* 컴파일 시점에서는 그 객체가 베이스 타입이라는 것만 알 수 있을 뿐이지 어떤 파생 타입인지는 정확히 알 수 없기 때문
		* 즉, 실행 시점에만 그 객체의 타입을 알아낼 수 있음
			* 이런 문제를 해결하기 위해 객체지향 언어에서는 [동적 바인딩](#동적-바인딩) 개념 사용

##### [목차로 이동](#목차)

#### 동적 바인딩
<img src="../img/polymorphism_uml.jpg" width="350" height="200"></br>

위 클래스 다이어그램에서 새의 종류에는 거위도 있고, 펭귄도 있다. 그리고 거위는 아장아장 걸어가거나 날아갈 수도 있고 물위를 헤엄칠 수도 있다. 반면에 펭귄은 뒤뚱뒤뚱 걷거나 물위를 헤엄친다. 모습은 서로 달라도 move()하는 것은 같다.

따라서 조류 조련사의 입장에서는 객체가 어떤 종류의 새든지 간에 그냥 새라고 생각하고 move()하라고 하면 된다. 즉, 어떤 타입의 새인지 알아내기 위해 별도의 코드를 작성할 필요가 없는 것이다. 그렇다면 이때 **거위나 펭귄이 어떻게 자신의 move()를 실행할 수 있는 것일까?**

객체지향이 아닌 종전의 프로그래밍 언어에서는 early 바인딩(binding)이라는 형태로 함수를 호출해야 한다(early 바인딩은 정적 바인딩이라고도 한다). 즉 새의 특정 타입인 거위나 펭귄이 갖는 move() 함수를 미리 지정하여 어떤 함수를 호출하는지 컴파일러가 알 수 있어야 한다는 것이다. 그리고 컴파일이 끝난 후 링커(linker)에 의해서 실행할 함수의 메모리 번지가 부여된다. 그러나 객체지향 프로그래밍에서는 컴파일이 아닌 실행하는 시점에서 호출될 함수의 메모리 번지가 결정된다. 따라서 이와 같이 베이스 타입의 객체에게 메시지를 보낼 때는 무언가 다른 형태의 처리가 필요하게 된다.

이러한 문제를 해결하기 위해 객체지향 언어에서는 late 바인딩 개념을 사용한다(late 바인딩은 동적 바인딩이라고도 한다). 즉, 어떤 파생 타입의 move()를 호출할 것인지를 실행할 때 결정하는 것이다. 컴파일러에서는 단지 그러한 함수가 있는지, 그리고 [함수의 인자와 반환 타입이 맞는지만](https://github.com/nara1030/ThisIsJava/blob/master/docs/etc/polymorphism.md)을 확인한다. 어떤 move()를 호출해야 하는지를 모르는 것이다.

자바에서는 호출할 함수의 메모리 번지를 바로 주지 않고 특별한 코드를 이용해서 late 바인딩을 구현한다. 그리고 그 코드에서는 객체의 정보를 기반으로 실행 함수의 번지를 계산한다. 따라서 각 객체는 자신의 타입에 있는 함수를 실행할 수 있는 것이다. C++ 언어에서 late 바인딩 시 별도의 키워드(virtual) 지정이 필요함과는 달리 자바에서는 기본적으로 late 바인딩을 하므로, 다형성을 구현하기 위해 별도의 키워드를 알고 있을 필요가 없다.
